# Watch topos and ERPs for frontal LPP
...
watch_topos <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block %in% c("Neu_Watch", "Pos_Watch", "Neg_Watch"),
         electrode %in% topo_elec, between(ms, 1000, 1300)) %>%
  group_by(block, electrode, x, y) %>%
  dplyr::summarize(amplitude = mean(amplitude, na.rm = TRUE)) %>%
  mutate(highlight = if_else(electrode %in% Pz_elec, "white", "black"),
         block = if_else(block == "Neu_Watch", "Neutral",
  ...
  scale_fill_viridis_c(limits = c(-4, 4), oob = scales::squish) +
...
watch_lpp <- eeg_dat %>%
  ...
  geom_vline(xintercept = c(1000, 1300), linetype = "solid", size = 1.05) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  annotate("rect", xmin = 1000, xmax = 1300, ymin = -Inf, ymax = Inf, alpha = .2) +
  labs(x = "Time (ms)",
...
ggsave("images/paper_2/front_lpp_watch_topos.png")
