...
tmp <- bind_rows(read_table2("data/paper_two/SPN mul/206201831_av-export.mul", skip = 1),
  ...
  mutate(pid = 206201831,
         block = rep(c("Pre_Pos_Inc",
                       "Pre_Pos_Dec",
                       "Pre_Pos_Watch",
                       "Pre_Neu_Watch",
                       "Pre_Neg_Inc",
                       "Pre_Neg_Dec",
                       "Pre_Neg_Watch"), each = nrow(.) / 7),
         ms = rep(seq(from = -200, to = 2000,
                      by = ((2200 + (2200 / (nrow(.)/7))) / (nrow(.)/7))),
                  times = 7)
  ) %>%
...
evt <- bind_rows(spn_evt, split_case_evt)
...
spn <- full_join(spn_dat, evt, by = c("pid", "block")) %>%
