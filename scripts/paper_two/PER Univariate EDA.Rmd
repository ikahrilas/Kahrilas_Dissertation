...
lpp_elec <- c("B28") # look at 450 - 800 ms and 800 - 2000 ms windows
...
SPN <- spn_wide %>% 
  select(all_of(spn), pid:prop_trials) %>% 
  filter(between(ms, 315, 1250)) %>% 
  pivot_longer(., cols = all_of(spn), names_to = "electrode", values_to = "mv") %>% 
  group_by(pid, block) %>% 
...
per_erp$LPP[per_erp$block == "Neg_Watch" & per_erp$LPP > 40] <- NA
per_erp$LPP_late[per_erp$block == "Neg_Watch" & per_erp$LPP_late > 50] <- NA
per_erp$LPP_late[per_erp$block == "Neg_Dec" & per_erp$LPP_late < -40] <- NA
per_erp$LPP[per_erp$block == "Pos_Watch" & per_erp$LPP < -5] <- NA
per_erp$LPP_late[per_erp$block == "Neg_Watch" & per_erp$LPP_late > 30] <- NA
per_erp$SPN[per_erp$block == "Neu_Watch" & per_erp$SPN < -20] <- NA
per_erp$SPN[per_erp$block == "Pos_Dec" & per_erp$SPN < -20] <- NA
