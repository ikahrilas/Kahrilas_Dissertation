...
epn <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block %in% c("Neg_Dec", "Neg_Watch", "Neg_Inc"),
         electrode %in% topo_elec, between(ms, 223, 325)) %>%
...
n170_epn_topos <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block %in% c("Neg_Dec", "Neg_Watch", "Neg_Inc"),
         electrode %in% topo_elec, between(ms, 112, 217)) %>%
  group_by(block, electrode, x, y) %>%
  dplyr::summarize(amplitude = mean(amplitude, na.rm = TRUE)) %>%
  mutate(highlight = if_else(electrode %in% n170_epn_elec, "white", "black"),
         block = if_else(block == "Neg_Dec", "Decrease",
...
neg_n170_epn <- eeg_dat %>%
  ...
  geom_vline(xintercept = c(112, 217), linetype = "solid", size = 1.05) +
  geom_vline(xintercept = c(223, 325), linetype = "solid", size = 1.05) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  annotate("rect", xmin = 112, xmax = 217, ymin = -Inf, ymax = Inf, alpha = .2) +
  annotate("rect", xmin = 223, xmax = 325, ymin = -Inf, ymax = Inf, alpha = .2) +
  labs(x = "Time (ms)",
...
ggsave("images/paper_2/epn_n170_neg_topos.png")
