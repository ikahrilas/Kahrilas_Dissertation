...
eeg_dat$B28[eeg_dat$pid == 206201854 & eeg_dat$block %in% c("Neg_Watch", "Neg_Dec") & between(eeg_dat$ms, 400, 2000)] <- NA
...
# merge coordinates with ERP data
eeg_dat <- read_csv(here("data", "paper_two", "created_data", "erp_mast.csv"))
eeg_dat$A5[eeg_dat$pid == 206201817 & eeg_dat$block %in% c("Neu_Watch", "Pos_Dec") & between(eeg_dat$ms, 450, 1215)] <- NA
eeg_dat_long <- pivot_longer(eeg_dat, cols = A1:EXG2, names_to = "electrode", values_to = "amplitude")
eeg_dat_loc <- left_join(eeg_dat_long, elec_loc, by = c("electrode" = "channel"))
topo_elec <- c(paste0("A", 1:32), paste0("B", 1:32))
...
```{r SPN-pos-topos, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "(ref:SPN-reg_tpops)", cache = TRUE, cache.lazy = FALSE}
library(devtools)
library(eegUtils) # remotes::install_github("craddm/eegUtils")
library(tidyverse)
library(here)
