...
As explained in more detail in the method section, we implemented a principal components analysis (PCA) approach to identifying ERP components, and previous research on the neural correlates of reactivity and regulation contributed to informing our selection of components.
The present study is inherently exploratory given the utilization of PCA to identify ERP components.
...
We also outline expected patterns of results regarding these components.
...
We expected enhanced N170-like activity in response to both positive and negative stimuli.
...
Given the limited research on explicit emotion regulation and EPN/N170, analyses regarding regulation blocks were conducted in an entirely exploratory manner.
...
Thus, the LPP is sensitive to the arousal and valence of emotional stimuli, and we expected augmented LPP in response to both positive and negative images.
...
However, a recent study conducted by Wilson and MacNamara [-@wilsonSavorMomentWillful2021], that found increases in centroparietal and frontal LPP when participants were asked to savor (i.e., upregulate) positive images compared to passively viewing images.
Thus, we expected LPP modulations congruent with the regulatory goals.
Further, we surmised taht higher self-report ratings of one’s capacity to savor would be expected to moderate the relationship between LPP and increased emotional intensity in response to positive stimuli.
<!-- Methodological factors might be contributing to these findings, such as inconsistent stimulus presentation durations, utilization of non-blocked vs. blocked designs, and offline filter settings. -->
<!-- LPP is theorized to be indicative of an interaction between amygdala and prefrontal structures [@moserElectrophysiologicalCorrelatesDecreasing2009] as indicated by LPP amplitude being sensitive to reappraisal efforts [e.g., reduced LPP amplitude in response to reappraising negative visual stimuli; @moserIntentionalModulationEmotional2006]. -->
...
### The Present Study
The present study utilized an experimental paradigm using 120 different standardized complex affective stimuli. 
Forty images were highly arousing positive images, 40 images were highly arousing negative images, and 40 were neutral. 
Participants were given directions with increasing or decreasing emotional intensity in response to (i.e., upregulating or downregulating) positive and negative images, as well as passively watching them. 
Participants also passively viewed the block of 40 neutral images for a total of seven blocks.
The present study had two primary objectives.
The first objective was to examine how block type would influence ERP components using an exploratory PCA approach.
We expected that highly arousing positive and negative compared to neutral OASIS stimuli would be associated with enhanced ERP components retained from the PCA (e.g., N170, EPN, LPP).
Related, we expected reduction of LPP in decrease conditions relative to passive watch conditions, and increased LPP in increase relative to passive watch conditions.
Analyses of regulatory effects on ERP/N170 were conducted in an exploratory manner.
The second objective was to examine the relation between personality traits and EEG components.
We expected that a) PA would moderate the relationship between watching positive images and LPP and b) savoring capacity would moderate the relationship between increasing emotional intensity in response to positive images and LPP. 
Moderation analyses of personality traits and N170/EPN were exploratory.
...
The International Affective Picture System [IAPS; @langInternationalAffectivePicture2008] has been used in a majority of the aforementioned research.
...
Thus, there is an increasing demand for images with standardized arousal and arousal ratings for behavioral researchers to use in online contexts, such as the OASIS.
