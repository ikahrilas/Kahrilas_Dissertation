...
```{r LPP-topo-erp-plots, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "(ref:LPP-topo-eorp-plots)"}
library(devtools)
...
erp_topo <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block == "Pos_Inc", electrode %in% topo_elec, between(ms, 450, 800)) %>%
  group_by(electrode, x, y) %>%
  summarize(amplitude = mean(amplitude, na.rm = TRUE))

topoplot(data = erp_topo, limits = c(-5, 5), palette = "viridis", highlights = c("A25", "B28", "B22")) +
  ggtitle("Positive Increase") +
  theme(plot.title = element_text(hjust = 0.5, size = 22))

# testing for contrast amplitudes
lpp_topo_plot <- function(block_1, block_2, title) {
tmp <- eeg_dat_loc %>% 
...
dat <- left_join(tmp, tmp_2, by = c("pid", "electrode", "ms")) %>% 
  ...
  group_by(electrode, x, y) %>%
  summarize(amplitude = mean(amplitude, na.rm = TRUE))
topoplot(dat, palette = "viridis", highlights = c("A25", "B28", "B22")) +
  ggtitle(title) +
  theme(plot.title = element_text(hjust = 0.5, size = 22))
}
```
