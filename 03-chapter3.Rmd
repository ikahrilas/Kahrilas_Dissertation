...
```{r corr-table, echo = FALSE, message = FALSE, fig.cap = "(ref:corr-table)"}
# load packages
library(tidyverse)
library(xtable)
library(kableExtra)
...
tab <- dat %>% 
  ...
  mutate(" " = colnames(.)) %>% 
  select(` `, everything()) %>% 
  rstatix::replace_upper_triangle(., by=  "", diagonal = FALSE)
colnames(tab) <- c(" ", paste0(c(1:8), "."))

# create table
kable(tab, format = "latex",  linesep = "", digits = 2, escape = FALSE, booktabs = TRUE, align = c("l", rep("r", 8)), caption = "Correlations among affectivity, temporal domains of savoring, depression severity, and anxious arousal measures ($N$ = 1,618)") %>% 
  column_spec(2:9, width = "2em") %>% 
