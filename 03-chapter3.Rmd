...
```
Trials without valid EEG data were excluded from analyses (*n* = 2 for N170/EPN/LPP anlayses and *n* = 5 for SPN). Analyses were run in R version 4.0.0 [@rcoreteamLanguageEnvironmentStatistical2018]. Multilevel modeling (MLM) analyses were conducted using the lme4 package [@batesFittingLinearMixedeffects2015]. All linear mixed models were estimated using REML with particpants included as random intercepts. Model assumptions were checked via visual inspection of diagnostic plots (i.e., residual, QQ, scale-location, and residuals vs. leverage) generated with the performance package [@ludeckePerformanceAssessmentRegression2019]. Correlations among key psychological and demographic variables (see Table 2\ref(tab:corr-table)) indicated significant associations ranging from small to large in effect size. Descriptive statistics for LPP, EPN, N170 and participant ratings (arousal, valence, and difficulty) were calculated for each of the experimental blocks (see Table \@ref(tab:descriptives-by-block)). A repeated measures analysis of variance yielded significant variation among experimental image blocks for LPP ($F$(`r df_num_lpp`, `r df_den_lpp` = `r f_lpp`, $p < .001$), EPN ($F$(`r df_num_epn`, `r df_den_epn`) = `r f_epn`, $p < .001$), and N170 ($F$(`r df_num_n170`, `r df_den_n170`) = `r f_n170`, $p < .001$). Repeated measures analysis of variance similarly indicated differences among blocks for arousal ($F$(`r df_num_ar`, `r df_den_ar` = `r f_ar`, $p < .001$), valence ($F$(`r df_num_val`, `r df_den_val` = `r f_val`, $p < .001$), and difficulty ($F$(`r df_num_diff`, `r df_den_diff`) = `r f_diff`, $p < .001$) ratings. A significant effect of block was not found for SPN and late LPP. Pairwise comparisons were conducted between passive watch conditions for EEG components and behavioral ratings (Table \@ref(tab:pairwise-watch-comparison-table)) as well as regulation conditions (Table \@ref(tab:pairwise-reg-comparison-table)) using the emmeans package [@lenthEmmeansEstimatedMarginal2020]. Effect sizes were interpreted using Funder's [-@funderEvaluatingEffectSize2019] recommendations. Between watch conditions, results indicated large to very large differences for LPP (see Table \@ref(tab:pairwise-watch-comparison-table) & Figure \@ref(fig:LPP-watch-topos)), small to medium differences for EPN, and small to large differences for N170 (see Table \@ref(tab:pairwise-reg-comparison-table) & Figure \@ref(fig:EPN-N170-watch-topos)). Behavioral ratings were in the expected directions, with both positive and negative images resulting in higher arousal ratings compared to neutral images. Further, negative and positive image blocks resulted in lower (i.e., more negative) and higher (i.e., more positive) valence ratings, respectively. Notably, the differences in behavioral ratings associated with negative vs. neutral image blocks were larger than they were for positive vs. neutral image blocks. Between positive regulation conditions, results indicated no significant effects for LPP (see Table \@ref(tab:pairwise-reg-comparison-table)) and Figure \@ref(fig:LPP-pos-topos)), a very small difference between increase and watch conditions for EPN (see Table \@ref(tab:pairwise-reg-comparison-table) & Figure \@ref(fig:EPN-N170-pos-topos)), and no significant differences for N170. Increase conditions resulted in very small and medium increases in arousal and valence ratings, respectively, compared to passive watch conditions. Decrease conditions led to decreases in arousal and valence ratings (Table \@ref(tab:pairwise-reg-comparison-table)). Between negative regulation conditions, results indicated a small difference between increase and watch conditions for LPP (see Table \@ref(tab:pairwise-reg-comparison-table) & Figure \@ref(fig:LPP-neg-topos)) and no significant differences for EPN and N170 (see Table \@ref(tab:pairwise-reg-comparison-table) & Figure \@ref(fig:EPN-N170-neg-topos)). Increase conditions resulted in tiny and large increases for arousal and valence ratings in increase vs. watch and decrease vs. watch conditions, respectively (Table \@ref(tab:pairwise-reg-comparison-table)).
...
(ref:EPN-N170-watch-topos) Topographical plots and average waveforms for N170 (160 - 200 ms; top row) and EPN (250 - 375 ms; bottom row) components during passive watch conditions. Sensors highlighted in white were used to measure N170 and EPN for waveform plots and analyses.
...
erp_topo <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block == block_type, electrode %in% topo_elec, between(ms, 160, 200)) %>%
  group_by(electrode, x, y) %>%
  dplyr::summarize(amplitude = mean(amplitude, na.rm = TRUE)) %>% 
...
ggplot(erp_topo, aes(x = x, y = y, fill = amplitude)) +
  ...
  geom_head() +               
  geom_channels(aes(color = highlight), size = 0.1) +             
...
erp_topo <- eeg_dat_loc %>%
  filter(!is.na(amplitude), block == block_type, electrode %in% topo_elec, between(ms, 250, 375)) %>%
  group_by(electrode, x, y) %>%
  dplyr::summarize(amplitude = mean(amplitude, na.rm = TRUE)) %>% 
  mutate(highlight = if_else(electrode %in% epn_n170_elec, "white", "black"))
ggplot(erp_topo, aes(x = x, y = y, fill = amplitude)) +
...
```{r EPN-N170-neg-topos, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "(ref:EPN-N170-neg-topos)", cache = TRUE, cache.lazy = FALSE}
rm(list=ls())
library(devtools)
library(eegUtils) # remotes::install_github("craddm/eegUtils")
library(tidyverse)
...
```{r positive-affectivity-mlm-analyses, include = FALSE}
rm(list=ls())
library(tidyverse)
library(lmerTest)
library(effectsize)
...
dat <- read_csv("data/paper_two/created_data/per_data_analyses_2020_5_19.csv") %>% 
  mutate(N170 = (N170_left + N170_right) / 2,
         ...
         pos_affectivity = scale(pos_affectivity, scale = FALSE, center = TRUE)) %>% 
  select(pid:LPP, N170, EPN, everything())
...
dat <- read_csv("data/paper_two/created_data/per_data_analyses_2020_5_19.csv") %>% 
  mutate(N170 = (N170_left + N170_right) / 2,
         EPN = (EPN_left + EPN_right) / 2,
         savoring_moment = scale(savoring_moment, scale = FALSE, center = TRUE)) %>% 
  select(pid:LPP, N170, EPN, everything())
