...
kable(., format = "latex",  linesep = "", escape = FALSE, booktabs = TRUE, align = c("l", rep("r", 9)), caption = "(ref:corr-table)") %>% 
  ...
  footnote(general_title = "Note.",
           escape = FALSE,
           general = "*$p < .05$. PA = positive affectivity, StM = savoring the moment. EEG components averaged across all regulation blocks.",
           threeparttable = TRUE,
...
kable(., "latex", escape = FALSE, booktabs = TRUE, align = c("l", "c", "c", "c"), linesep = "", caption = "(ref:pairwise-watch-comparison-table)") %>% 
  ...
  footnote(escape = FALSE,
           footnote_as_chunk = TRUE,
           general_title = "Note.",
           general = "Std. Beta (Label) = Standardized Beta coefficient as measure of effect size derived by fitting multilevel model to standardized dataset with effect size label as per Funder's (2019) recommendations, Sig. = $p$ value. $P$ values and confidence intervals adjusted using the Tukey method for comparing a family of three estimates.",
           threeparttable = TRUE) %>% 
  save_kable("poop.png")
```
