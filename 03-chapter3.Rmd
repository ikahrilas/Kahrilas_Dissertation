...
```{r EPN-N170-neg-topos, echo=FALSE, fig.cap="(ref:EPN-N170-neg-topos)", message=FALSE, warning=FALSE, cache=TRUE, cache.lazy=}
rm(list=ls())
library(devtools)
library(eegUtils) # remotes::install_github("craddm/eegUtils")
library(tidyverse)
...
spn_pa_beta <- spn_pa_dat %>% 
  ...
  select(Estimate) %>% 
  pull()
spn_pa_beta <- standardize_parameters(spn_pa) %>% 
