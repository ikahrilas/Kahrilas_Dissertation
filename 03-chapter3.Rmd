...
As explained in more detail in the method section, we implemented a principal components analysis (PCA) approach to identifying ERP components [@dienApplyingPrincipalComponents2012; @donchinMultivariateAnalysisEventrelated1978], and previous research on the neural correlates of reactivity and regulation contributed to informing our selection of components.
...
the OAISS  was normed on arousal ratings collected from a diverse sample recruited through Amazon's Mechanical Turk [@kurdiIntroducingOpenAffective2017].
...
A key reasons that a block design was selected for this study was because arousing positive and negative images outnumber the neutral images two-to-one, and the lower arousing neutral stimuli may produce a P3 if intermixed with higher arousing (e.g., positive/negative) stimuli in the same block [@schuppAffectivePictureProcessing2000]. 
...
Each analysis resulted in a significant test statistic at $\alpha = .05$, signifying that block type accounted for a significant portion of the variance in each fo the retained ERP components and behavioral ratings. Follow up pairwise *t* tests were conducted using the emmeans package [@lenthEmmeansEstimatedMarginal2020] among passive watch and regulation conditions for PCA EEG components and behavioral ratings.
Reactivity effects were observed starting at the 162 ms component and largely continued through the time course until the 740 ms component.
Regulation effects for positive stimuli unfolded started at 259 ms with differences observed between passive viewing and increase conditions that were observed in the 381 ms component as well.
Regulation effects for negative stimuli started earlier at 124 ms, with differences observed between passive viewing and increase conditions that were also observed in the 259 ms component, and more detailed statistical results for EEG components are summarized in Figure \@ref(fig:contrast-plot).
...
```{r rm-anova-tab, message = FALSE, include = FALSE, echo = FALSE, fig.pos="H", fig.cap = "(ref:rm-anova-tab)", out.height = "100%", out.width = "100%"}
...
However, a small difference was noted for between the negative increase and negative watch conditions, with greater amplitudes for the increase relative to the watch condition.
...
There was a small difference between the positive watch and increase conditions, with greater 381 ms component amplitude in increase conditions, as well as an additional small difference between increase and decrease conditions.
...
An additional small effect was noted when comparing the positive increase and decrease conditions.

Regarding arousal behavioral ratings, there were significant reactivity effects when comparing negative and neutral conditions, $t(318) = 5.93,\ SE = 0.21,\ p <.001$ (medium sized effect), positive and neutral conditions, $t(318)=3.01,/ SE = 0.21,/ p = .008$ (small sized effect), as well as negative and positive conditions, $t(318)=2.92,/ SE = 0.21,/ p = .011$ (small sized effect). 
On average, participants rated negative images as more arousing than positive images and positive images as more arousing than neutral images.
Reactivity differences in valence ratings were also observed between negative and neutral conditions, $t(318) = 6.04,\ SE = 0.15,\ p <.001$ (medium sized effect), positive and neutral conditions, $t(318) = 3.08,\ SE = 0.15,\ p =.006$ (small sized effect), as well as positive and negative conditions, $t(318) = 9.12,\ SE = 0.15,\ p <.001$ (large sized effect). 
On average, participants rated negative images more valenced (i.e., farther away from the midpoint of the scale, which signified neutral valence) than positive images and neutral images and positive images more valenced than neutral images.
These patterns coincide with patterns of ERP component amplitudes, suggesting that there is overlap between these behavioral ratings and ERP components with regards passively viewing emotional stimuli.
In terms of positive regulation conditions, differences in arousal ratings were observed when comparing increase and watch conditions, $t(318) = 4.78,\ SE = 0.21,\ p <.001$ (medium sized effect), as well as increase and decrease conditions, $t(318) = 6.10,\ SE = 0.21,\ p <.001$ (large sized effect). 
No significant difference was noted between the decrease and watch conditions.
Differences in valence ratings were also noted when comparing increase and watch conditions $t(318) = 5.67,\ SE = 0.15,\ p <.001$ (medium sized effect), the decrease and watch conditions, $t(318) = 3.70,\ SE = 0.15,\ p <.001$ (small sized effect), as well as the decrease and increase conditions $t(318) = 9.37,\ SE = 0.15,\ p <.001$ (large sized effect).
In terms of negative regulation conditions, differences in arousal ratings were observed when comparing increase and watch conditions, $t(318) = 4.42,\ SE = 0.21,\ p < .001$ (medium sized effect), as well as increase and decrease conditions, $t(318) = 4.07,\ SE = 0.15,\ p < .001$ (medium sized effect).
There was no difference between the decrease and watch conditions
Valence rating differences also emerged when comparing increase and watch conditions $t(318) = 3.58,\ SE = 0.15,\ p =.001$ (medium-sized effect), as well as decrease and increase conditions $t(318) = 2.71,\ SE = 0.15,\ p =.019$ (medium sized effect).
...
(ref:contrast-plot) Pre- (top row of topographic plots) and post-PCA (second row of topographic plots) topographical plots collapsed across all blocks and statistical findings for pairwise *t* tests among each block for each component. Pre-PCA topographic plots represent average activity across the time window above each plot. Electrode sites used to measure each component highlighted in the post-PCA topographic plots. Differences among blocks are illustrated with box-and-whisker/violin plots below the topographic plots. The width of the violin plots corresponds to the number of cases, or density, with that level of the dependent variable. Regarding the box-and-whisker plots, the area between the lower and upper hinges correspond to the interquartile range (the 25th and 75th percentiles). 
The upper whisker extends from the hinge to the largest value no further than 1.5 times the interquartile range from the hinge. The lower whisker extends from the hinge to the smallest value at most 1.5 times the interquartile range of the hinge. Data beyond the end of the whiskers are are plotted individually as dots.
...
## DISCUSSION
We explored the neural time course of positive and negative emotion reactivity and regulation with an exploratory temporal PCA approach in the present study.
...
Regarding emotion regulation, different patterns emerged that were contingent upon whether the stimuli were positive or negative, and negative regulatory effects unfolded early in the time course at 124 ms while positive regulatory effects were observed starting at 259 ms.
Only increase effects were observed relative to passive watch conditions, and no decrease effects were found.
Overall, this suggests that emotion reactivity unfolds more consistently throughout the neural time course, while regulatory effects are smaller and their time course contingent upon valence.
In devising neuroscience-informed interventions, whether the target is reactivity, positive emotion regulation, or negative emotion regulation may have important implications in terms of modifiable neural correlates.
This research will also help introduce more up-to-date and open access materials in affective neuroscience research and devise neuroscience-informed clinical interventions to enhance wellbeing.
...
However, it is the point at which we first observed an increase (relative to passive viewing) effect for negative images.
This finding further challenges the notion of emotion regulation implying change from *initial* reactivity [@lewisAdvancesProblemsChallenges2010a] and begs the question: does emotion reactivity necessarily precede regulation?
...
Modifiable neural mechanisms of negative emotion regulatory processes, then, may occur earlier in the time course.

<!-- 162 ms component -->
For the next extracted component at 162 ms, passively viewing positive and negative OASIS stimuli elicited amplitude relative to neutral stimuli.
Within the the present study, this marks the time point at which reactivity processes begin to unfold throughout the rest of the neural time course of extracted PCA components (i.e., the first 1000 ms following stimulus presentation).
We also found that negative images elicited augmented amplitude relative to positive images, ultimately illustrating both an arousal and valence effect at this component (and, generally, all subsequent components).
While no previous studies have investigated components this early in the context of emotion reactivity to complex affective images, this does fit with research illustrating larger amplitude for positive slow wave components [e.g., @fotiDifferentiatingNeuralResponses2009; @frankHemodynamicElectrocorticalReactivity2019].
No regulatory effects were found for this component, implying that activity within this time window is specific to emotion reactivity.
This also illustrates that negative emotion regulation is not necessarily a continuous process.
Rather, it is implicated at certain points in the time course regardless of the topographical similarities of the component.
In sum, interventions targeted at emotion reactivity may consider neural processes starting very early in the time course and through the time course.
Emotion regulation, on the other hand, may require a more selective approach in identifying related neural activity.
...
This contradicts a supposed "pleasure bias" observed in another negative peak component often observed in this time window, the EPN [@frankHemodynamicElectrocorticalReactivity2019].
...
Another study conducted in our lab (Kahrilas et al., in prep) suggests that positive reactivity in this component is related to positive affectivity (i.e., one's disposition to experience positive emotions).
Therefore, a lack of a positive reactivity effect may be attributed to the fact that the present sample was not recruited on the basis of depressive symptoms.
Concerning regulatory effects, significant differences were observed for both positive and negative stimuli.
To the authors' knowledge, this is the first display of explicit regulatory effects in at this time point, ultimately suggesting that early visual ERP processes may play a larger role in emotion regulation than previously surmised.
Of note, only increase effects were observed.
It may be that this component is specific to increasing emotional intensity, or that our paradigm was unsuccessful in eliciting decrease effects, as indicated by null findings in other ERP components and behavioral arousal ratings.
...
However, our results did align with Wilson and MacNamara (2021), who adopted a novel paradigm that instructed participants to savor positive IAPS stimuli, with language borrowed from Bryant's [@bryantFourfactorModelPerceived1989; @bryantSavoringNewModel2007] conceptualization of savoring.
...
While positive and negative image sets were matched on normative arousal and arousal ratings in the present study, pornographic images were omitted, which are commonly rated as highly arousing positive images [@bradleyInternationalAffectivePicture2007:1; @kurdiIntroducingOpenAffective2017] and used in other studies investigating neural time course of affective processing with positive images [e.g., @hajcakTemporalDynamicsEmotion2007; @liuNeuralSubstrateLate2012].
Thus, the omission of these images may be contributing to discrepant findings in the present study compared to others.
With regards to reactivity, it appears that the 162 ms, 381 ms, and 740 ms component mapped onto behavioral ratings of valence and arousal, or state mood manipulations

Regarding regulatory conditions, positive and negative increase conditions resulted in enhanced arousal/arousal ratings compared to passive watch conditions.
This pattern of positive regulation ratings matches differences noted in the 259 ms and 381 ms components, while the 124 ms and 259 ms component matches the behavioral ratings of negative images.
It appears that the relation among ERP component and state mood manipulation is contingent upon the type of emotion.
Decrease conditions only resulted in attenuated valence ratings for positive, and not negative, images.
...
### Conclusions
In conclusion, our study illustrated that a cascade of early and slow wave ERP components that were modified in reactivity conditions, with more selective modulations in regulation conditions that were contingent upon stimulus valence.
These are important considerations in devising neuroscience-informed treatments for those with dysregulated emotion reactivity and/or regulation.
Additionally, this is one of the first studies to examine the neural time course of affective processing using the OASIS.
...
Future aims have been identified based on these findings, such as examining relations among self-report data and ERP data, recruiting a more diverse sample, and utilizing different EEG methods (e.g., source analysis, time frequency analysis).
