...
The temporal PCA used all time points (-200 ms - 2000 ms) as variables and all 113 participants, three blocks, and 64 recording sites as observations, yielding linear combinations of time points and "temporal" factors [@dienApplyingPrincipalComponents2012].
...
```{r raw-pos-257-component-plot, echo = FALSE, fig.pos="H", out.extra="", fig.cap = "(ref:raw-pos-257-component-plot)", out.height= "45%", out.width="100%"}
...
Note that the positive attenuation and negative potentiation views are not mutually exclusive, thus these patterns could be observed simultaneously.
...
(ref:sem-diagrams) The model diagrams show each model that was fitted to the data for each ERP component retained from the PCA. The Measurement Model (top) treats the latent *Neural Reactivity* and *Internalizing* factors as orthogonal constructs. The ECI Model (second from top) estimates covariances among the positive/negative viewing conditions for a given ERP component and positive affectivity. The Neural Reactivity Model (third from top) estimates covariance between the neural reactivity latent factor (rather than specific emotional reactivity) and positive affectivity. The Internalizing Model Diagram (second from bottom) estimates covariances among positive and negative viewing conditions for a given ERP PCA component and global internalizing symptoms, rather than PA specifically. Last, the Anxiety Model (bottom) estimates covariances among positive/negative viewing conditions for a given ERP component and anxious arousal. PA = positive affectivity, Anx Aro = anxious arousal, Anx App = Anxious apprehension, ECI = emotion context insensitivity.

```{r sem-diagrams, echo = FALSE, fig.pos="H", out.extra="", fig.cap = "(ref:sem-diagrams)", out.height= "120%", out.width="120%"}
knitr::include_graphics("images/paper_3/sem_diagrams/sem_diagrams_final.png", auto_pdf = TRUE)
```

All models were estimated using full information maximum likelihood with robust (Huber-White) standard errors and a scaled test statistic. 
...
Inspection of missing cases did not reveal any indications that these data were missing not at random [@endersIntroductionMissingData2010].
...
A chi-square difference test will be performed between the nested measurement model and its corresponding ECI, neural reactivity, internalizing, and anxiety model.
...
The ECI, Neural Reactivity, Internalizing, and Anxiety Model are not hierarchical and thus cannot be comaprd using a chi-square difference test.
...
No significant relation was found between the 257 ms component and PA.
...
The Neural Reactivity model provided a signficantly better fit to the data compared to the nested measurement model ($\chi^2(1) = 13.96,\ p<.001$).
...
In this model, we dropped the covariance estimate between the negative viewing condition and PA (i.e., constrained the value to zero), which is in line with the brightening effect described in ecological momentary assessment (EMA) research [e.g., @khazanovBrighteningEffectReactions2019a].
In contrast to laboratory research, the brightening effect pertains to the phenomenon of enhanced emotion reactivity that is specific to positive emotions.
...
Additional, this brightening effect model did not result in an appreciable decrement in model fit compared to the ECI model, which has an additional parameter estimate and one fewer degree of freedom ($\Delta\chi^2(1) = 3.38,\ p=.07$).
...
We examined three separate ERP components retained from a PCA analysis, one that resembled an EPN and two others resembling later components (e.g., P300 and LPP).
...
Our analytic strategy was similar to that of Hill and colleagues (2019), though we implemented some distinct elements in the present paper.
...
Additionally, we took a adopted a more granular approach in measure depression symptomatology by focusing on PA, as informed by literature surmising a distinct relation between this construct and emotion reactivity (CITATIONS HERE).
...
While we hypothesized results resembling that of previous laboratory research [e.g., @hillAbnormalEmotionalReactivity2019] and supporting the ECI view, our findings did not support this pattern of findings.
With regards to the 371 ms and 736 ms component, no competing model outperformed the initial measurement model.
In other words, our results supported the notion that these components are distinct from internalizing symptoms.
...
Our analytic strategy also differs from a majority of other studies in that we utilized PCA and SEM.
Indeed, other studies measuring windowed averages are susceptible to including superimposed ERP signals in their independent/dependent variables, particularly for slow wave components such as the LPP (CITATION).
Thus, researchers may not always be measuring the constructs they believe they are.
...
Last, this finding could also simply be a product of type II error that can only be clarified with further research.
